---
name: scaffold-chart-domain
description: Scaffold a new chart domain in the echarts-rn project. Creates the domain folder under src/charts/domains/<name>/, generates a payload source (either a sample JSON matching ChartDataPayload or a build<Name>Payload.ts client-side calculator), a screen component patterned after HomeValueScreen, the matching charts.<name>.* i18n keys in src/i18n/en.ts, and wires the screen into App.tsx. Use when the user wants to add a new chart domain, scaffold a new chart screen, or asks how to add a new chart.
---

# Scaffold a new chart domain

Project conventions you must respect live in `AGENTS.md` at the repo root. Read that first if you haven't.

## Phase 1 — Gather inputs

Ask the user via `AskUserQuestion`:

1. **Domain name** (camelCase, e.g., `creditScore`). This becomes the folder name `src/charts/domains/<name>/`, the i18n namespace `charts.<name>`, and the screen component `<Name>Screen`.
2. **Data source**:
   - **API fetch** — generate a `sample-<name>-response.json` matching `ChartDataPayload` (raw API shape, `sourceMapKeys` + tuple `source`).
   - **Client-side compute** — generate a `build<Name>Payload.ts` that returns `ChartDataPayload` directly. Used when no API call is justified (e.g., amortization).
...
Run `npm run typecheck` via Bash. If it fails, fix the issue (most often: missing i18n key, wrong dimension `role`, or a typo in `seriesKeys`). Report status concisely. Do **not** run `npm test` unless the user asks — generated samples don't have unit tests by default.

## Invariants this skill enforces

- **Never edit** `packages/charts/contract/`, `packages/charts/builders/`, or `packages/charts/utils/`. Those are shared; domains must conform to them.
- **Every `labelKeys` reference** in the payload must resolve to a real key in `src/i18n/en.ts`.
- **Hooks** (`useQuery`, `useTranslation`, `useChartTypeFlag`) live only at the top of the screen component, never inside builders or transformers.
- **No `any` types**.
